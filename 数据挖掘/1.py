...
def loadDataSet():
    ...
    """
    # 对数据进行处理
    dataSet = pd.read_excel('1804-13.xlsx')
    test_dataSet = pd.DataFrame(pd.read_excel('test_dataSet.xlsx'))
    test_dataSet = test_dataSet.drop('class', axis=1)
    labelSet = list(dataSet.columns.values)
    dataSet = dataSet.values
    test_dataSet = test_dataSet.values
    return dataSet, labelSet, test_dataSet
...
def classifyAll(myTree, featLabels, testDataSet):
     """
     输入：决策树，分类标签，测试数据集
     输出：决策结果
     描述：跑决策树
     ...
     for testVec in testDataSet:
         classLabelAll.append(classify(myTree, featLabels, testVec))
     return classLabelAll

if __name__ == '__main__':
    dataSet, labelSet, test_dataSet = loadDataSet()
